...
import { logout, profile, profileCoverImage, profileImage, removeProfileImage, rotateToken, shareProfile, updatePasswrd } from "./user.service.js";
...
router.patch("/password", authentication(),validation(validators.updatePasswrd) ,async (req, res, next) => {
    const credentials   = await updatePasswrd(req.body, req.user, `${req.protocol}://${req.host}`)
    return successResponse({ res, data: { ...credentials   } })
